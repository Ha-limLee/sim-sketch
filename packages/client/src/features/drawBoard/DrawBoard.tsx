...
const createEventMapper: CreateEventMapper = (
  dispatch,
  draft,
  createDraft,
  isDrawing,
  ...
  line: {
    ...
    handleMouseMove: (e) => {
      ...
      dispatch(
        setDraft({
          ...draft,
          points: [...points.slice(0, 2), offsetX, offsetY],
        }),
      );
    },
    handleMouseUp: (e) => {
...
type DraftProp = NodeProp & {
  handleClick?: (evt: KonvaEventObject<MouseEvent>) => void;
  handleAnchorDragEnd?: (anchor: number[]) => void;
  isDrawing?: boolean;
};
...
type CreateDraftMapper = (
  ...
  nodes: createdNode[],
  selectedShape: ShapeType,
) => { [T in ShapeType]?: DraftProp };

const createDraftMapper: CreateDraftMapper = (
  draft,
  dispatch,
  nodes,
  selectedShape,
  ...
  curve: {
    ...
    handleAnchorDragEnd(anchor) {
      dispatch(
        setNodes([
          ...nodes,
          { shape: selectedShape, prop: { ...draft, anchorPoint: anchor } },
        ]),
      );
      dispatch(setIsDrawing(false));
    },
  ...
  return (
    ...
    >
      <Layer>
        ...
        {isDrawing
          ? nodeBuilder[selectedShape](draftMapper[selectedShape] ?? draft)
          : null}
      </Layer>
    </Stage>
  );
